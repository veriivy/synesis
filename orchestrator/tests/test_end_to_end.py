...
from orchestrator.coding import TicketCode
...
GENERATED_MARKER = "# generated by the coding agent"
UNOWNED_PATH = "src/audit_trail.py"


def fake_agent_implementer(provider_for=None):
    """Stands in for execution.agent_implementer's real model call. Writes
    plausible content for every file the ticket owns, and — like a real
    model that overreaches — one file it does not own, so the e2e run also
    exercises write_file's refusal through the live app."""

    async def implement(room, ticket) -> TicketCode:
        body = "def handle_{tid}() -> None:\n    return None\n"
        files = {
            path: f"{GENERATED_MARKER} for {ticket.ticket_id}\n"
            + body.format(tid=ticket.ticket_id)
            for path in ticket.files_owned
        }
        files[UNOWNED_PATH] = "# a path this ticket does not own\n"
        return TicketCode(files=files)
...
def fake_llm_calls(monkeypatch):
    ...
    monkeypatch.setattr(main_module, "agent_implementer", fake_agent_implementer)
...
@pytest.mark.asyncio
async def test_execution_writes_the_coding_agents_output_and_refuses_an_unowned_path(
    client: AsyncClient,
):
    """The point of the coding agent: what lands in the workspace is the
    code the agent produced, not a stub — and a path it does not own is
    refused by write_file even though the agent asked for it."""
    room_id, tokens = await _create_room_with_tasks(client, priority_u1="must", priority_u2="want")
    ...
    for user_id in ("u1", "u2"):
        await client.post(
            f"/rooms/{room_id}/plan/approve",
            json={"user_id": user_id, "approved": True, "participant_token": tokens[user_id]},
        )
    await client.post(f"/rooms/{room_id}/execute")
    await asyncio.wait_for(room.execute_task, timeout=5)

    # Every owned file holds the agent's generated content.
    for ticket in room.tickets:
        for path in ticket.files_owned:
            f = (await client.get(f"/rooms/{room_id}/files/{path}")).json()
            assert GENERATED_MARKER in f["content"], f"{path} did not get generated code"
            assert "TODO: implement" not in f["content"]

    # The unowned path the agent reached for was refused, and never written.
    writes = [e for e in main_module.bus.history(room_id) if e["type"] == "file_written"]
    refused = [e for e in writes if not e["accepted"]]
    assert refused, "expected the unowned write to be refused"
    assert all(e["path"] == UNOWNED_PATH for e in refused)
    assert "path_not_owned" in refused[0]["reason"]
    assert (await client.get(f"/rooms/{room_id}/files/{UNOWNED_PATH}")).status_code == 404
    # ...and being refused does not fail the ticket that owns its own files.
    assert {t.status for t in room.tickets} == {"done"}
